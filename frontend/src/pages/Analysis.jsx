  ...
  return (
    <div className="flex flex-col gap-1 w-56 shrink-0">
      <div className="flex w-full h-2.5 rounded-full overflow-hidden gap-px">
        <div
          className="bg-pitch-600 transition-all"
          style={{ width: `${p1}%` }}
        />
        <div
...
function MatchupRow({ matchup }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl px-5 py-4 flex items-center justify-between gap-4 shadow-sm">
      <div
        className={`w-36 text-right text-sm font-semibold ${w1 ? "text-pitch-700" : "text-slate-400"}`}
      >
        {matchup.team1}
        {w1 && (
          <span className="ml-1.5 text-[10px] bg-pitch-100 text-pitch-700 px-1.5 py-0.5 rounded font-normal">
            wins
          </span>
        )}
      </div>
      <ProbBar p1={p1} p2={p2} />
      <div
        className={`w-36 text-sm font-semibold ${!w1 ? "text-pitch-700" : "text-slate-400"}`}
      >
        {!w1 && (
          <span className="mr-1.5 text-[10px] bg-pitch-100 text-pitch-700 px-1.5 py-0.5 rounded font-normal">
            wins
          </span>
        )}
        {matchup.team2}
  ...
  return (
    <section className="mb-14">
      <div className="flex items-baseline gap-3 mb-5">
        <span className="text-pitch-700 font-bold text-lg">{number}.</span>
...
function StatCard({ label, value, sub }) {
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 text-center shadow-sm">
      <div className="text-3xl font-bold text-pitch-700 mb-1">{value}</div>
...
function FeatureCard({ name, desc }) {
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
      <div className="text-sm font-semibold text-pitch-700 mb-1.5">{name}</div>
...
function ChallengeCard({ title, problem, solution }) {
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
      ...
      <div className="mb-2">
        <span className="text-[10px] font-bold uppercase tracking-wider text-red-500 mr-2">
          Problem
        </span>
        <span className="text-sm text-slate-600">{problem}</span>
      </div>
      <div>
        <span className="text-[10px] font-bold uppercase tracking-wider text-pitch-700 mr-2">
          Solution
        </span>
...
function ArchBox({ label, items, color }) {
  const colors = {
    green: "border-pitch-300 bg-pitch-50",
    blue: "border-gold-300 bg-gold-50",
    amber: "border-gold-300 bg-gold-50",
    slate: "border-slate-300 bg-slate-50",
  };
  const textColors = {
    green: "text-pitch-700",
    blue: "text-gold-700",
    amber: "text-gold-700",
  ...
  return (
    <div className={`border rounded-xl p-4 ${colors[color]}`}>
      <div
        className={`text-xs font-bold uppercase tracking-wider mb-2 ${textColors[color]}`}
      >
        {label}
      </div>
      {items.map((item) => (
        <div key={item} className="text-sm text-slate-700 py-0.5">
          {item}
        </div>
      ))}
    </div>
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="mb-16 pt-2">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-pitch-700 mb-5">
          Machine Learning · Full-stack Engineering
        </p>
        <h1 className="text-4xl sm:text-6xl font-bold text-slate-900 leading-[1.04] tracking-tight mb-6">
          Predicting the 2026
          <br className="hidden sm:block" /> World Cup with ML
        </h1>
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <span
              key={tag}
              className="text-xs font-medium text-slate-500 border border-slate-200 px-3 py-1 rounded-full"
            >
              {tag}
            </span>
          ))}
        </div>
        <p className="text-xs text-slate-400 mt-6">By Amr Abujabal · 2026</p>
      </div>
      ...
      <Section number="Abstract" title="">
        <p className="text-slate-600 leading-relaxed text-base">
          This project is an end-to-end machine learning system for predicting
          international football match outcomes — home win, draw, or away win —
          and simulating the 2026 FIFA World Cup knockout bracket. The ML
          pipeline covers data ingestion, chronological feature engineering (ELO
          ratings, recent form, head-to-head records), XGBoost training with
          leakage-free backtesting, and a live prediction API. On top of this
          sits a full-stack web application: a FastAPI backend with a PostgreSQL
          database and a React frontend with an interactive bracket challenge
          and real-time leaderboard.
        </p>
        <p className="text-slate-600 leading-relaxed text-base mt-4">
          Backtesting against the 2018 and 2022 World Cups yields ~40% accuracy
          — meaningfully above the 33.3% random baseline for a 3-class problem
          and comparable to published academic models on the same task.
        </p>
      </Section>
      ...
      <Section number="1" title="System Architecture">
        ...
        <p className="text-slate-600 text-sm leading-relaxed">
          The model is loaded once on the first API request using{" "}
          <code className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-700 font-mono text-xs">
            lru_cache(maxsize=1)
          </code>{" "}
          — this singleton pattern avoids re-training on every request while
          keeping the deployment stateless. Training takes ~30 seconds on cold
          start; subsequent predictions are &lt;100ms.
        </p>
      </Section>
      ...
      <Section number="2" title="Data">
        <p className="text-slate-600 text-base leading-relaxed mb-6">
          The dataset is sourced from Kaggle's international football results
          archive, covering every recorded international match from 1872 to 2024
          across 194 nations. Each row contains the home team, away team, final
          score, tournament name, and a neutral-ground flag. The full dataset is
          loaded with memory-efficient dtypes (
          <code className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-700 font-mono text-xs">
            category
          </code>{" "}
          for team/tournament columns,{" "}
          <code className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-700 font-mono text-xs">
            int16
          </code>{" "}
          for scores) to reduce RAM footprint.
        </p>
        ...
        <p className="text-slate-600 text-sm leading-relaxed">
          All feature computation is done in a single chronological pass over
          the sorted dataset. This is the critical design constraint that
          prevents data leakage — ELO ratings, form windows, and H2H records are
          always computed from matches strictly before the one being predicted.
        </p>
      </Section>
      ...
      <Section number="3" title="Feature Engineering">
        <p className="text-slate-600 text-base leading-relaxed mb-4">
          Nine features are computed per match using only data available before
          kick-off. The feature set is intentionally lean — three conceptual
          groups capture the signal that matters for football prediction without
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-5 text-sm text-slate-600 leading-relaxed">
          ...
          The entire feature matrix is built in a single forward pass, sorted by
          date. ELO ratings are updated only after each match is processed,
          never before. This guarantees that predictions at inference time use
          the same information available at training time.
        </div>
        <div className="grid sm:grid-cols-3 gap-3 mb-5">
          {[
            {
              name: "ELO Difference",
              desc: "Skill gap between the two teams. K-factor is weighted by tournament importance: FIFA WC = 60, Confederations Cup = 50, Qualifier = 40, Friendly = 20.",
            },
            {
              name: "Home ELO",
              desc: "Home team's absolute ELO rating, computed chronologically from a starting value of 1500 using a standard Elo update formula.",
            },
            {
              name: "Away ELO",
              desc: "Away team's absolute ELO rating on the same chronological pass — captured separately since relative skill and absolute level both matter.",
            },
            {
              name: "Home Form",
              desc: "Average points per game over the home team's last 5 matches (3 = win, 1 = draw, 0 = loss), capturing short-run momentum.",
            },
            {
              name: "Away Form",
              desc: "Average points per game over the away team's last 5 matches — momentum is asymmetric, so home and away form are kept separate.",
            },
            {
              name: "H2H Home Wins",
              desc: "Home team wins in the last 10 direct encounters, capturing psychological head-to-head dynamics between specific nations.",
            },
            {
              name: "H2H Draws",
              desc: "Draws in the last 10 direct encounters. Some pairings are structurally draw-prone; this captures that pattern.",
            },
            {
              name: "H2H Away Wins",
              desc: "Away team wins in the last 10 head-to-head encounters between these two sides.",
            },
            {
              name: "Is Neutral",
              desc: "Binary flag: 1 if played on neutral ground. Home advantage is real in football; all WC bracket matches are set to 1.",
            },
          ].map((f) => (
      ...
      <Section number="4" title="Model">
        <p className="text-slate-600 text-base leading-relaxed mb-4">
          XGBoost was chosen for its proven strength on tabular data, native
          3-class probability output, and fast training on CPU. Neural
          approaches (e.g. a simple MLP) were prototyped but showed no accuracy
          improvement on this dataset while being considerably slower to iterate
          on. The model outputs softmax probabilities across three classes —
          home win, draw, and away win — via{" "}
          <code className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-700 font-mono text-xs">
            multi:softprob
          </code>
          .
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-xl p-5 font-mono text-sm text-slate-300 leading-loose mb-5">
          <span className="text-pitch-400">XGBClassifier</span>(<br />
          &nbsp;&nbsp;n_estimators=<span className="text-yellow-300">300</span>
          ,&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          <span className="text-slate-500">
            # enough trees without overfitting
          </span>
          <br />
          &nbsp;&nbsp;max_depth=<span className="text-yellow-300">5</span>
          ,&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          <span className="text-slate-500">
            # shallow trees — football is noisy
          </span>
          <br />
          &nbsp;&nbsp;learning_rate=
          <span className="text-yellow-300">0.05</span>
          ,&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          <span className="text-slate-500">
            # slow learning, better generalisation
          </span>
          <br />
          &nbsp;&nbsp;subsample=<span className="text-yellow-300">0.8</span>
          ,&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          <span className="text-slate-500">
            # row subsampling to reduce variance
          </span>
          <br />
          &nbsp;&nbsp;colsample_bytree=
          <span className="text-yellow-300">0.8</span>,&nbsp;
          <span className="text-slate-500"># feature subsampling per tree</span>
          <br />
          &nbsp;&nbsp;eval_metric=
          <span className="text-blue-400">'mlogloss'</span>,<br />
          &nbsp;&nbsp;objective=
          <span className="text-blue-400">'multi:softprob'</span>,&nbsp;
          <span className="text-slate-500"># 3-class probability output</span>
          <br />
        ...
        <p className="text-slate-600 text-sm leading-relaxed">
          Class labels are encoded as 0 = home win, 1 = draw, 2 = away win. The
          model is serialised with{" "}
          <code className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-700 font-mono text-xs">
            joblib
          </code>{" "}
          after training and loaded as a singleton at API startup. Per-class
          probabilities are exposed directly in the API response so the frontend
          can render live win-probability bars on every matchup.
        </p>
      </Section>
      ...
      <Section number="5" title="Backtest Results">
        <p className="text-slate-600 text-base leading-relaxed mb-6">
          Backtesting uses a strict temporal split: the model is trained on all
          matches before the World Cup year, then evaluated on that tournament's
          group stage and knockout rounds. This is the only honest evaluation
          protocol — any leakage of future data into training would inflate
          accuracy.
        </p>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm text-sm text-slate-600 leading-relaxed space-y-2">
          <p>
            ...
            A 3-class random classifier scores 33.3%. A naïve "always predict
            home win" classifier scores ~45% on historical data but fails on
            neutral-ground WC matches. The model's 39.8% on neutral-ground WC
            matches is therefore competitive.
          </p>
          <p>
            <span className="font-semibold text-slate-800">
              Draw difficulty:
            </span>{" "}
            Draws are the hardest class — they make up ~25% of outcomes but are
            structurally hard to predict. A model that ignores draws would score
            higher on raw accuracy but would be useless for bracket simulation.
            The draw class is retained deliberately.
          </p>
          <p>
            <span className="font-semibold text-slate-800">ELO dominance:</span>{" "}
            Feature importance analysis shows ELO difference is the strongest
            single predictor, accounting for ~40% of split gain. This aligns
            with football analytics literature — skill gap is the best long-run
          ...
          {[
            {
              step: "01",
              title: "Live Bracket",
              desc: "The bracket page shows the official FIFA WC 2026 knockout bracket — Round of 32 through the Final. Live scores and match status (LIVE / FT) are fetched from the backend and overlaid on every card, auto-refreshing every 60 seconds. The AI's win probabilities (renormalized to exclude draws for knockout rounds) are shown on each matchup, and completed results lock in the bracket automatically.",
              tags: [
                "32 teams",
                "live scores",
                "60s auto-refresh",
            ...
            <div
              key={f.step}
              className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex gap-5"
            >
              <div className="text-3xl font-black text-slate-200 shrink-0 leading-none mt-0.5">
                {f.step}
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {f.tags.map((t) => (
                    <span
                      key={t}
                      className="text-[10px] font-semibold bg-pitch-50 text-pitch-700 border border-pitch-200 px-2 py-0.5 rounded-full"
                    >
                      {t}
                    </span>
                  ))}
      ...
      <Section number="8" title="Model's 2026 Bracket Predictions">
        <p className="text-slate-600 text-base leading-relaxed mb-8">
          ...
          neutral-ground (is_neutral = 1). Predictions are fetched live from the
          backend.
        </p>
        ...
        {bracket && (
          <div className="mt-10 border-2 border-gold-400 bg-gold-50 rounded-2xl p-10 text-center shadow-[0_4px_24px_rgba(251,191,36,0.2)]">
            <div className="text-5xl mb-4">🏆</div>
            <div className="text-xs font-bold uppercase tracking-widest text-gold-600 mb-2">
              Model's Predicted Champion
            </div>
      ...
      {/* Challenge CTA */}
      <div className="my-14 rounded-2xl border border-slate-200 bg-slate-50 px-8 py-10">
        <div className="text-center mb-8">
          <p className="text-xs font-bold uppercase tracking-widest text-pitch-700 mb-3">
            Your Turn
          </p>
          ...
          {[
            {
              ...
              items:
                "Python 3.12 · pandas · XGBoost · scikit-learn · joblib · numpy",
            },
            {
              layer: "Backend API",
              items:
                "FastAPI · SQLAlchemy 2.0 · Pydantic v2 · uvicorn · python-dotenv",
            },
            {
              layer: "Database",
              items:
            ...
            },
            {
              layer: "Frontend",
              items:
                "React 18 · Vite · Tailwind CSS v3 · React Router v6 · axios",
            },
            {
              layer: "Deployment",
              items:
            ...
            },
            {
              layer: "Dataset",
              items:
                "Kaggle — International Football Results 1872–2024 (49,287 matches)",
            },
          ].map((s) => (
            <div
              key={s.layer}
